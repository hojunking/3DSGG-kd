...
class Mmgnet(BaseModel):
    def __init__(self, config ,num_obj_class, num_rel_class, dim_descriptor=11, tconfig= None):
        ...
        self.momentum = 0.1
        self.model_pre = None
        self.initial_point_dim = self.mconfig.point_feature_size
        # Object Encoder
        ...
        self.rel_encoder_2d = PointNetfeat(
            ...
            input_transform=False,
            feature_transform=mconfig.feature_transform,
            out_size=self.mconfig.edge_feature_size)
        ...
        self.triplet_projector_2d = torch.nn.Sequential(
            #redu
            torch.nn.Linear(self.mconfig.point_feature_size * 2 + self.mconfig.edge_feature_size, self.mconfig.point_feature_size * 2),
            torch.nn.Dropout(0.5),
            torch.nn.ReLU(),
            torch.nn.Linear(self.mconfig.point_feature_size * 2, 512)
        )
        ...
        self.mlp_3d = torch.nn.Sequential(
            torch.nn.Linear(512 + 256, self.mconfig.point_feature_size - 8),
            torch.nn.BatchNorm1d(self.mconfig.point_feature_size - 8),
            torch.nn.ReLU(),
            torch.nn.Dropout(0.1)
        )
        if self.mconfig.point_feature_size == 256:
            self.mlp_2d = torch.nn.Sequential(
                torch.nn.Linear(512, self.mconfig.point_feature_size),
                torch.nn.BatchNorm1d(self.mconfig.point_feature_size),
                torch.nn.ReLU(),
                torch.nn.Dropout(0.1)
            )
        ...
        if mconfig.multi_rel_outputs:
            self.rel_predictor_3d = PointNetRelClsMulti(
                num_rel_class,
                #redu
                in_size=self.mconfig.edge_feature_size, 
    ...
    def init_weight(self, obj_label_path, rel_label_path, adapter_path):
        ...
        self.obj_predictor_2d = torch.nn.Linear(self.mconfig.point_feature_size, self.num_class)
        self.obj_predictor_2d.weight.data.copy_(obj_text_features)
        for param in self.obj_predictor_2d.parameters():
            param.requires_grad = True
        
        self.obj_predictor_3d = torch.nn.Linear(self.mconfig.point_feature_size, self.num_class)
    ...
    def get_label_weight(self, obj_label_path, rel_label_path):
        ...
        if self.mconfig.point_feature_size == 256:
            linear_layer = torch.nn.Linear(512, 256).cuda()  # 512 차원을 256 차원으로 변환하는 선형 레이어
            obj_text_features = linear_layer(obj_text_features)
        return obj_text_features, rel_text_features.float()
    ...
    def kd_process_train(self, teacher, obj_points, obj_2d_feats, gt_cls, descriptor, gt_rel_cls, edge_indices, batch_ids=None, with_log=False, ignore_none_rel=False, weights_obj=None, weights_rel=None, beta = 0.1):
        ...
        if self.mconfig.multi_rel_outputs:
            ...
            ############ KD ############
            t_loss_rel_3d = F.binary_cross_entropy(rel_pred,t_rel_pred, weight=weight)
        ...
        # feature + loss
            ...
            t_loss_obj, t_loss_rel = t_mse_loss_obj + obj_feature_loss, t_loss_rel_3d + rel_feature_loss
        else:
            t_loss_obj, t_loss_rel = t_mse_loss_obj, t_loss_rel_3d

        obj_loss = lambda_o * (loss_obj_2d + (alpha_o * loss_obj_3d  + beta_o * t_loss_obj))
        rel_loss = 3 * lambda_r * (loss_rel_2d + (alpha_r * loss_rel_3d + beta_r * t_loss_rel))
        mimic_loss = 0.1 * (loss_mimic + rel_mimic_2d)
        loss = obj_loss + rel_loss + mimic_loss
        ...
        if self.kd_method == 'fkl' or self.kd_method == 'fmse':
        ...
        if self.kd_method == 'fmse':
            log.insert(8, ("train/T_bce_loss_rel", t_loss_rel_3d.detach().item()))
