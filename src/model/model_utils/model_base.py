...
class BaseModel(nn.Module):
    ...
    def save(self):
        ...
        print('Saving model to %s' % os.path.join(self.saving_pth, self.name + suffix))
    ...
    def load(self, best=False):
        ...
        if best:
            print(f'best model loaded: iteration: {self.iteration}, eva_res: {self.eva_res}')
        for name,model in self._modules.items():
            skip = False
            for k in self.skip_names:
                if name.find(k) != -1:
            ...
            if skip is False:
                ...
                if best:
                    print(f'loading {name} from {os.path.join(self.saving_pth, name + suffix)}')
